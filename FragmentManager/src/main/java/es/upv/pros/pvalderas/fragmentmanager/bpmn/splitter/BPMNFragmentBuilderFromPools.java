...
public class BPMNFragmentBuilderFromPools implements BPMNFragmentBuilder{
	...
	protected BPMNFragmentBuilderFromPools(String compositionName, Node poolParticipant, XMLQuery query) throws JaxenException{
	...
	private Document getNewPool(Node poolParticipant) throws JaxenException{
		...
		this.fragmentId=compositionName.replaceAll(" ", "")+"_"+microserviceId.replaceAll(" ", "")+"_fragment"; //modified
	...
	private Node getProcessByID(String processID) throws JaxenException{
		...
		for(Node task:tasks){
			if(task.valueOf("@camunda:delegateExpression").length()==0)
	...
	private Node getDiagram(Element root, String participantId) throws JaxenException{
		...
		for(String message:this.messageFlows){
			...
			if(id==null){
				...
				shape=query.selectSingleNode("//bpmndi:BPMNShape[@bpmnElement='"+id+"']");
				Node shapeBounds=query.selectSingleNode("//bpmndi:BPMNShape[@bpmnElement='"+id+"']/dc:Bounds");

				x1=String.valueOf(Integer.parseInt(shapeBounds.valueOf("@x"))+Integer.parseInt(shapeBounds.valueOf("@width"))/2);
				y1=String.valueOf(Integer.parseInt(shapeBounds.valueOf("@y"))+Integer.parseInt(shapeBounds.valueOf("@height")));
				...
				x2=x1;
				y2=String.valueOf(Integer.parseInt(shapeBounds.valueOf("@y"))+Integer.parseInt(shapeBounds.valueOf("@height")));
			}
			...
			try{
				String color=shape.valueOf("@ns0:stroke");
				if(color!=null){
					edge.addAttribute("ns0:stroke", color);
					edge.addAttribute("xmlns:ns0","http://bpmn.io/schema/bpmn/biocolor/1.0");
				}
			}catch(XPathException  e){
				//System.out.println("There is no Stroke att");
			}
